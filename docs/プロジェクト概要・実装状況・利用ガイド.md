...
flowchart TD
    ...
    Crawler --> Mode{--traversal}
    Mode -->|dfs| DFS[_crawl_dfs<br/>再帰的深さ優先]
    Mode -->|bfs| BFS[_crawl_bfs<br/>deque 幅優先]
    DFS --> Fetch[_process_url]
    BFS --> Fetch
    Fetch --> HTTP[requests GET]
    HTTP --> Parse[BeautifulSoup<br/>リンク・title 抽出]
    Parse --> Results[results リスト]
...
2. `Crawler.crawl()` が `--traversal` に応じて DFS または BFS でリンクを辿る
...
| 機能 | 実装 | 詳細 |
|---|---|---|
| 再帰的クローリング | ✅ | `<a href>` からリンクを抽出し DFS または BFS で巡回 |
| 巡回方式の切り替え（DFS / BFS） | ✅ | `--traversal dfs`（デフォルト）/ `--traversal bfs` |
| Cookie ファイル認証 | ✅ | `--cookies` で Netscape 形式（Get cookies.txt LOCALLY 互換） |
...
`localhost_8000.csv` の結果例（**DFS**、`--traversal dfs`）:
...
同じ test-server を **BFS**（`--traversal bfs`）でクロールした場合:

```csv
url,title,status,depth
http://localhost:8000,Test Home,200,0
http://localhost:8000/page1.html,Test Page 1,200,1
http://localhost:8000/page2.html,Test Page 2,200,1
http://localhost:8000/index.html,Test Home,200,2
http://localhost:8000/sub/page3.html,Test Page 3,200,2
```

> **DFS と BFS の違い:** DFS ではリンクの出現順によって深度が直感的でない場合があります（上記 DFS 例では `page2.html` が depth 3）。BFS では開始 URL からの最短リンク数に近い深度になり、サイトマップの階層把握に向いています。
...
| `--traversal` | `dfs` / `bfs` | `dfs` | 巡回方式。`bfs` は幅優先（深度が最短リンク数に近い） |
| `--cookies` | ファイルパス | なし | Netscape 形式の Cookie ファイル |
...
# 階層把握向けに BFS で実行
uv run main.py http://localhost:8000 --output localhost_8000_bfs --traversal bfs
```

#### サイトマップ用途（BFS 推奨）

```bash
# 開始URLからの階層深度を安定させたい場合
uv run main.py https://example.com --traversal bfs --excel
```
...
ブラウザ拡張機能 [Get cookies.txt LOCALLY](https://github.com/kairi003/Get-cookies.txt-LOCALLY) で Netscape 形式をエクスポートし、ログイン済みセッションでクロールします。

```bash
# 1. ブラウザで対象サイトにログイン
# 2. 拡張機能で Netscape 形式をエクスポート → cookies.txt
# 3. クロール実行
uv run main.py https://example.com/mypage \
  --cookies ./cookies.txt \
  --traversal bfs
```

> `cookies.txt` は認証情報を含むため Git にコミットしないでください。リポジトリの `.gitignore` で以下が除外されています:
>
> - `cookies.txt`
> - `**/cookies.txt`
> - `*.cookies.txt`
>
> ファイル名を変えて保存した場合は、手動で `.gitignore` に追加するか、`--cookies` で任意パスを指定してください。
...
| `depth` | 整数 | 開始 URL からのリンク辿り深度（0 = 開始ページ）。BFS では最短リンク数に近い値になる |
...
| パラメータ | デフォルト | 説明 |
|---|---|---|
| `traversal` | `"dfs"` | `"dfs"`（深さ優先）または `"bfs"`（幅優先） |
| `cookie_file` | `None` | Netscape 形式 Cookie ファイルのパス |
...
| `traversal` | 巡回方式（`dfs` / `bfs`） |

**主要メソッド:**

| メソッド | 役割 |
|---|---|
| `crawl()` | `traversal` に応じて `_crawl_dfs()` または `_crawl_bfs()` を呼び出す |
| `_crawl_dfs(url, depth)` | 再帰による深さ優先探索 |
| `_crawl_bfs()` | `collections.deque` による幅優先探索 |
| `_process_url(url, depth)` | HTTP 取得・結果記録・BeautifulSoup 返却 |
| `_extract_links(soup, base_url)` | `<a href>` からリンク URL を抽出 |
| `_reserve_url(url)` | 許可・未訪問チェック後に `visited_*` を更新 |
| `_load_netscape_cookies(path)` | `MozillaCookieJar` で Cookie ファイルを `Session` に読み込み |

**HTTP クライアント:**

- `requests.Session()` を全リクエストで使い回す
- `--cookies` 指定時は Netscape 形式（Get cookies.txt LOCALLY 出力互換）を読み込む
...
**DFS と BFS の visited 管理:**

- **DFS:** 各 URL の処理開始時に `_reserve_url()` で訪問済みにする
- **BFS:** キューへ追加（enqueue）する時点で `_reserve_url()` し、二重 enqueue を防ぐ
...
| Basic 認証非対応 | HTTP Basic 認証が必要なサイトは不可（Cookie 認証は `--cookies` で対応） |
| シングルスレッド | 大規模サイトでは完了まで時間がかかる |
| DFS 時の深度のずれ | デフォルト DFS では `depth` が辿った経路に依存する（`--traversal bfs` で改善可能） |
| DFS 時の再帰呼び出し | 極端に深いサイトではスタックオーバーフローのリスク（`--max-depth` または `--traversal bfs` で回避） |
...
### 5.4 Cookie ファイルのセキュリティ

- `cookies.txt` はブラウザのログインセッションをそのまま含む **機密ファイル** です
- `.gitignore` で `cookies.txt` / `**/cookies.txt` / `*.cookies.txt` を除外済み
- `git add -f` による強制追加や、別名ファイル（例: `my_site_cookies.txt`）は除外対象外になるため注意
- チーム共有が必要な場合は、Cookie ファイルではなくステージング用のテストアカウント情報を別チャネルで管理することを推奨
...
# テストサーバー起動（別ターミナル）
uv run python -m http.server 8000 --directory test-server

# 短い待機で素早く確認（DFS）
uv run main.py http://localhost:8000 --delay-min 0.1 --delay-max 0.2 --output test_run_dfs

# BFS で深度の違いを確認
uv run main.py http://localhost:8000 --delay-min 0.1 --delay-max 0.2 --traversal bfs --output test_run_bfs
```

### 7.2 コード変更時の注意点

- `Crawler.crawl()` は `traversal` に応じて `_crawl_dfs()` / `_crawl_bfs()` に分岐する
- BFS では `_reserve_url()` を enqueue 時に呼び出す（DFS は処理開始時）
- `unique_path` と `visited_paths` の更新は `_reserve_url()` 内で行われる
...
| 2026-06-30 | BFS 対応（`--traversal bfs`）の追記 |
| 2026-06-30 | Cookie ファイル認証（`--cookies`）の追記 |
| 2026-06-30 | `.gitignore` の Cookie 除外ルールとセキュリティ注意の追記 |
